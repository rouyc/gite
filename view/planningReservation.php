		      	...
		      	<div class="col s12 l6">
			        <div class="card">
			          	<div class="card-content">
			          		<table>
								<tr> 
									<td colspan="7"><a href="./index.php?controller=chambre&action=build_chambre&id=<?php echo $_GET["id"]?>&mois=<?php echo $num_mois-1; ?>&amp;annee=<?php echo $num_an; ?>"><<</a>&nbsp;&nbsp;
                                        <?php echo $tab_mois[$num_mois];  ?>&nbsp;&nbsp;
                                        <a href="./index.php?controller=chambre&action=build_chambre&id=<?php echo $_GET["id"]?>&mois=<?php echo $num_mois+1; ?>&amp;annee=<?php echo $num_an; ?>">>></a></td>

									<td class="right"><a href="./index.php?controller=chambre&action=build_chambre&id=<?php echo $_GET["id"]?>&mois=<?php echo $num_mois; ?>&amp;annee=<?php echo $num_an-1; ?>"><<</a>&nbsp;&nbsp;
                                        <?php echo $num_an;  ?>&nbsp;&nbsp;
                                        <a href="./index.php?controller=chambre&action=build_chambre&id=<?php echo $_GET["id"]?>&mois=<?php echo $num_mois; ?>&amp;annee=<?php echo $num_an+1; ?>">>></a></td>
								</tr>
								<?php
									echo'<tr>';
									for($i = 1; $i <= 7; $i++){
									...
									for($i=0;$i<6;$i++) {
										echo "<tr>";
										for($j=0;$j<7;$j++) {
                                            $dateA = $num_an . " " . $num_mois .  " " . $tab_cal[$i][$j];
                                            ...
                                            if ($t==0) {
                                                echo "<td class =\" green accent-4\"".(($num_mois == date("n") && $num_an == date("Y") && $tab_cal[$i][$j] == date("j"))?' style="color: #FFFFFF; background-color: #000000;"':null).">".((strpos($tab_cal[$i][$j],"*")!==false)?'<font color="#aaaaaa">'.str_replace("*","",$tab_cal[$i][$j]).'</font>':$tab_cal[$i][$j])."</td>";
                                            } else {
                                                echo "<td class =\" red accent-4\"".(($num_mois == date("n") && $num_an == date("Y") && $tab_cal[$i][$j] == date("j"))?' style="color: #FFFFFF; background-color: #000000;"':null).">".((strpos($tab_cal[$i][$j],"*")!==false)?'<font color="#aaaaaa">'.str_replace("*","",$tab_cal[$i][$j]).'</font>':$tab_cal[$i][$j])."</td>";
                                            }
